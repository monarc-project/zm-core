            ...
            if (!is_null($value) && !empty($value) && !is_object($value)) {
                if ($metadata->hasAssociation($propertyname)) {
                    ...
                    if (!is_array($value) || isset($value['id']) || isset($value['uuid'])) {
                        if (isset($value['uuid']) || (\is_string($value) && Uuid::isValid($value))) {
    ...
    protected function manageRelativePositionUpdate($field, $entity, $direction)
    {
        ...
        if ($direction == 'up') {
          try{
            ...
            $entityAbove = $table->getEntityByFields([$field => ['uuid' => $entity->$field->getUuid(), 'anr'=>$entity->$field->anr->id], 'position' => $entity->position - 1]);
          }
          ...
          try{
            ...
            $entityBelow = $table->getEntityByFields([$field => ['uuid' => $entity->$field->getUuid(), 'anr'=>$entity->$field->anr->id], 'position' => $entity->position + 1]);
          }
                      ...
                      //file_put_contents('php://stderr', print_r($this->get('table'), TRUE).PHP_EOL);
                        if(in_array('anr',$this->get($key.'Table')->getClassMetadata()->getIdentifierFieldNames())){
                          if(isset($data['anr']) && $data['anr'] != null) // TO IMPROVE fo with uuid
                            $data[$key] = ($entity->$key) ? ['uuid' => $entity->$key->getUuid(), 'anr' => $data['anr'] ] : null;
                          else if($entity->$key->anr !=null && $entity->$key->anr->id !=null )
                            $data[$key] = ($entity->$key) ? ['uuid' => $entity->$key->getUuid(), 'anr' => $entity->$key->anr->id ] : null;
                          }
                          else // bo with uuid
